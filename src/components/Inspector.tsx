...
// Only these are true image fields (for preview)
...
// Map alias field -> title field per module key
function getTitleFieldForAlias(moduleKey: string, aliasId: string): string | null {
  switch (moduleKey) {
    case "image_full_width":
      if (aliasId === "link_alias") return "image_title";
      return null;
        ...
        <div className="mt-6 space-y-8">
          ...
          <div className="space-y-4">
            {def.fields.map((field) => {
              ...
              const titleFieldId = getTitleFieldForAlias(module.key, field.id);
              const isAlias = !!titleFieldId;

              // If it's an alias field, only show once the associated title has a value
              if (isAlias) {
              ...
              return (
                <div key={field.id} className="space-y-1">
                  ...
                  {/* Optional to just render notes as small text blocks */}
                  {field.type === "note" && (
                  ...
                  {/* IMAGE PREVIEW (ONLY for true image fields) */}
                  {isImageField && value && (
                    <div className="mt-1">
                      <img
